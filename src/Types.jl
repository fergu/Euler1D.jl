...
- nedges::Int: The number of zone edges in the simulation, equal to nzones + 1. [ ⋅ ]
...
- x::Vector{T}: A vector of locations of zone edges. Increases monotonically. [ m ]
- Δx::Vector{T}: A vector of the size of each zone. [ m ]
- gamma::Vector{T}: A vector of the ratio of specific heats inside each zone. [ ⋅ ]
- mass::Vector{T}: A vector of the mass contained within each zone. Assumed constant. [ kg ]
- density::Vector{T}: A vector of the density of the fluid within each zone. Computed using the equation of state as mass/Δx. [ kg/m³ ]
- velocity::Vector{T}: A vector of velocities of zone edges. [ m/s ]
- pressure::Vector{T}: A vector of pressures inside each zone. Computed from the equation of state. [kg/(m⋅s²)]
- intenergy::Vector{T}: A vector of the internal energy per unit mass of each zone. [ m²/s² ]
- speedofsound::Vector{T}: A vector of the speed of sound within each zone. Computed from the equation of state. [ m/s ]
- viscosity::Vector{T}: A vector of the artificial viscosity within each zone, added to the pressure field. See ArtificialDissipation.jl for more information. [ kg/(m⋅s²) ]
- energy_flux::Vector{T}: A vector of fluxes of internal energy per unit mass across each zone boundary. Added to the energy equation as a diffusion term. See ArtificialDissipation.jl for more information. [ m³/s³ ]
- ∂u∂t::Vector{T}: A vector of the right hand side of the momentum equation at each zone edge. [ m/s² ]
- ∂e∂t::Vector{T}: A vector of the right hand side of the energy equation within each zone. [ m²/s³ ]
"""
struct Simulation{T}
